...
document.addEventListener("click", (event) => {
  if (
    !inicioBtnDoc.contains(event.target) &&
    !inicioMenuDoc.contains(event.target)
  ) {
...
document.addEventListener("click", (event) => {
  if (
    !inicioBtnPac.contains(event.target) &&
    !inicioMenuPac.contains(event.target)
  ) {
    inicioMenuPac.classList.add("hidden");
  }
});

// Botón y menú para Consulta
const inicioBtnCons = document.querySelector("#inicio-btn-consulta");
const inicioMenuCons = document.querySelector("#inicio-menu-consulta");

// Mostrar/Ocultar el menú al hacer clic en "Consulta"
inicioBtnCons.addEventListener("click", () => {
  inicioMenuCons.classList.toggle("hidden");
});

// Cerrar el menú si se hace clic fuera del botón o menú "Consulta"
document.addEventListener("click", (event) => {
  if (
    !inicioBtnCons.contains(event.target) &&
    !inicioMenuCons.contains(event.target)
  ) {
    inicioMenuCons.classList.add("hidden");
  }
});
